...
def get_model(latent_dim=8, sr=44100, duration=1.0, spectrogram_shape=(80, 1025), n_fft=2048):
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(16, 3, activation='tanh', padding='same'))(img_inputs)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(16, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(16, 3, activation='elu', padding='same'))(x)
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(16, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(16, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(16, 3, activation='linear', padding='same'))(x)
    x = layers.Flatten()(x)
    z_mean = layers.Dense(latent_dim, name="z_mean", activation=None)(x)
    z_log_var = layers.Dense(latent_dim, name="z_log_var", activation=None)(x)
    z = Sampling()([z_mean, z_log_var])
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(16, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(16, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(16, 3, activation='elu', padding='same'))(x)
