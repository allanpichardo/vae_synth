...
def get_model(latent_dim=8, sr=44100, duration=1.0, spectrogram_shape=(80, 1025), n_fft=2048):
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(24, 3, dilation_rate=1, activation='tanh', padding='same'))(img_inputs)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(24, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(24, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(36, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(36, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(36, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(54, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(54, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(54, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(81, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(81, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(81, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(121, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(121, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(121, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(182, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(182, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(182, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(273, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(273, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(273, 3, dilation_rate=4, activation='elu', padding='same'))(x)

    z_mean = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(1, 3, activation='linear', padding='same'))(x)
    z_mean = layers.Flatten(name="z_mean")(z_mean)
    z_log_var = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(1, 3, activation='linear', padding='same'))(x)
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(273, 3, dilation_rate=4, activation='tanh', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(273, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(273, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(182, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(182, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(182, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.ZeroPadding2D(padding=[(0, 1), (0, 0)])(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(121, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(121, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(121, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(81, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(81, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(81, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(54, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(54, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(54, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(36, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(36, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(36, 3, dilation_rate=1, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(24, 3, dilation_rate=4, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(24, 3, dilation_rate=2, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(24, 3, dilation_rate=1, activation='tanh', padding='same'))(x)
    x = layers.ZeroPadding2D(padding=[(0, 0), (0, 1)])(x)
    decoder_outputs = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(3, 3, activation=None, padding="same"))(x)
    decoder = keras.Model(latent_inputs, decoder_outputs, name="decoder")
