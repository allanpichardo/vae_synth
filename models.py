...
class VAE(keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            fft_phase_loss = tf.keras.losses.MeanSquaredError()(p, py)
            audio_reconstruction_loss = tf.keras.losses.MeanSquaredError()(data, audio_reconstruction)
...
def get_model(latent_dim=8, sr=44100, duration=1.0, spectrogram_shape=(80, 1025), n_fft=2048):
    ...
    x = kapre.composed.get_frequency_aware_conv2d(filters=8, kernel_size=3,  activation='tanh', padding='same')(img_inputs)
    # x = tf.keras.layers.Conv2D(24, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(24, 3,  activation='elu', padding='same')(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.Conv2D(12, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(36, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(36, 3,  activation='elu', padding='same')(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.Conv2D(18, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(54, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(54, 3,  activation='elu', padding='same')(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.Conv2D(27, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(81, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(81, 3,  activation='elu', padding='same')(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.Conv2D(41, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(121, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(121, 3,  activation='elu', padding='same')(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.Conv2D(62, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(182, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(182, 3,  activation='elu', padding='same')(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.Conv2D(93, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(273, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2D(273, 3,  activation='elu', padding='same')(x)
    ...
    x = tf.keras.layers.Conv2DTranspose(93, 3,  activation='tanh', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(273, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(273, 3,  activation='elu', padding='same')(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.Conv2DTranspose(62, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(182, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(182, 3,  activation='elu', padding='same')(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.ZeroPadding2D(padding=[(0, 1), (0, 0)])(x)
    x = tf.keras.layers.Conv2DTranspose(41, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(121, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(121, 3,  activation='elu', padding='same')(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.Conv2DTranspose(27, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(81, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(81, 3,  activation='elu', padding='same')(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.Conv2DTranspose(18, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(54, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(54, 3,  activation='elu', padding='same')(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.Conv2DTranspose(12, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(36, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(36, 3,  activation='elu', padding='same')(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.Conv2DTranspose(8, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(24, 3,  activation='elu', padding='same')(x)
    # x = tf.keras.layers.Conv2DTranspose(24, 3,  activation='tanh', padding='same')(x)
