...
def get_model(latent_dim=8, sr=44100, duration=1.0, spectrogram_shape=(80, 1025), n_fft=2048):
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(8, 3, activation='tanh', padding='same'))(img_inputs)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(8, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(8, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(12, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(12, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(12, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(18, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(18, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(18, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(27, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(27, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(27, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(40, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(40, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(40, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(60, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(60, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(60, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.MaxPooling2D()(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(90, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(90, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1D(90, 3, activation='elu', padding='same'))(x)
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(90, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(90, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(90, 3, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(60, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(60, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(60, 3, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.ZeroPadding2D(padding=[(0, 1), (0, 0)])(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(40, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(40, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(40, 3, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(27, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(27, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(27, 3, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(18, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(18, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(18, 3, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(12, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(12, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(12, 3, activation='elu', padding='same'))(x)
    x = layers.UpSampling2D(interpolation="nearest")(x)
    x = layers.ZeroPadding2D(padding=[(0, 0), (0, 1)])(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(8, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(8, 3, activation='elu', padding='same'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv1DTranspose(8, 3, activation='tanh', padding='same'))(x)
