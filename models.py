...
class VAE(keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # recon_shape = tf.shape(reconstruction)
            # padded = tf.concat([data, tf.zeros([data.shape[0], recon_shape[1] - data.shape[1], data.shape[2]])], 1)
...
def get_models_cnn(latent_dim=16, input_shape=(512, 1025, 2)):
    ...
    stft_shape = encoder.layers[1].output_shape
    div = pow(2, 5)
    t = int(stft_shape[1] // div)
    f = int(stft_shape[2] // div)

    latent_inputs = keras.Input(shape=(latent_dim,))
    x = layers.Dense(256, activation="relu")(latent_inputs)
    x = layers.Reshape((1, 2, 128))(x)
